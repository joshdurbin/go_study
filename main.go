...
func main() {
	root := &cobra.Command{
		Use:   "go_study",
		Short: "Go interview study guide: web UI + analytics",
	}

	root.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default: ./go_study.yaml)")
	root.PersistentFlags().String("db", "go_study.db", "SQLite database path")
	root.PersistentFlags().String("root", ".", "repo root containing basics/, patterns/, interview_study/")
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.AddConfigPath(".")
		viper.SetConfigName("go_study")
		viper.SetConfigType("yaml")
	}
	viper.SetEnvPrefix("GO_STUDY")
...
func serveCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "serve",
		Short: "Run the study guide web server (with analytics)",
